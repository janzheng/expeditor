...
**Totals (as of 2026-04-12): 29 fixed · 9 wontfix · 0 open · 2 split-status.** All remaining items closed in the cleanup-3 session (A010, A015, A024, A025). Prior wave on 2026-04-12 closed A008, A017, A018, A019 (refine/004-010); A028 and A029 fixed in c3c75a2 last session. See also `.brief/agentic-audit.md` for the newer (2026-04-12) agentic-UX audit with 16 findings, all shipped.
...
- [x] [fixed: harness-emit promise tracked via `harnessEmitPromise`; `done` block awaits it before resolving so the `failed` signal is persisted before `spawnAndWait` returns. SIGTERM now fires via `.finally()` after emit completes.] **A010** Unawaited `bus.emit()` in maxToolCalls subscriber `spawner.ts:710` #error-handling #local-real
...
- [x] [fixed: `.catch()` handler on the import promise logs the failure — no more silent unhandled rejection. cli.ts:1649] **A015** Webhook import fire-and-forget `cli.ts:1641` #error-handling #local-real
...
- [x] [fixed: reader typed as nullable, initialized to null; cancel() and pull() both no-op when reader is null so cancelled-before-start streams don't touch an uninitialized reference. bus.ts:357-405] **A024** `lineStream cancel()` crashes if reader uninitialized `bus.ts:262` #edge-case
- [x] [fixed: widened try/catch around entire pre-spawn path — buildCommand, Deno.Command construction, spawn(), stdin write all covered. settingsPath cleaned up on any failure before registry.register. spawner.ts:638-673] **A025** Temp sandbox dir leaks on spawn error before process created `spawner.ts:510,638` #resource-leak
...
- [x] **A008** Signals lost during log rotation — FIXED
- [x] **A011** validateCommand lifecycle — FIXED
- [x] **A014** Batch snapshot support — FIXED
- [x] **A017** Workflow synthesis gets empty output silently — FIXED
